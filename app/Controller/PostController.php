...
class PostController extends AppController {
    ...
    public function add() {
        ...
        /* If Admin only post enabled from admin added by babulal prasad v6.4 @22-jul-2019 */
        if (isset($loggedinUser['current_org']->only_admin_post) && $loggedinUser['current_org']->only_admin_post == 1) {
...
//                exit;
                ...
                $SCresponse = $this->Apicalls->curlpost("getOrgSubcenters.json", $postData);
                $SCresponse = json_decode($SCresponse, true);

                $subcenterArray = array();
                if (isset($SCresponse['result']['data'])) {
                    $subcenterArray = $SCresponse['result']['data'];
                }
...
//                        pr($this->request->data);
                        ...
                        if (isset($this->request->data['subcenter'])) {
                            $postData['subcenter_id'] = $this->request->data['subcenter'];
                        }
...
//                        exit;
        ...
        $this->set(compact("org_user_role", "subcenterArray"));
