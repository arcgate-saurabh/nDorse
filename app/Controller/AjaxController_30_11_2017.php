...
//                ))
        ...
        $orgdata = $this->Organization->find("all", array("fields" => array('Subscription.*', 'Organization.*'), 'joins' => $joins, "order" => "Organization.created DESC", "offset" => $totalrecords, "limit" => 20, "conditions" => $conditionsArray['conditions']));
...
//        pr($conditionsArray);
        $this->Organization->recursive = 2;
        $orgdata = $this->Organization->find("all", array("offset" => 0, "limit" => 20, "fields" => array('Subscription.*', 'Organization.*'), 'joins' => $joins, "order" => "Organization.created DESC", "conditions" => $conditionsArray['conditions']));
//        $log = $this->Organization->getDataSource()->getLog(false, false);
//        echo "<hr>";
//        pr($log);
//        exit;
    ...
    function saveasspreadsheetallposts() {
        ...
        if ($information == "allposts") {
            try {
                ...
                if ($type == "both") {
                    $result = array("Post", "User", "Title", "Department", "Sub Org", "Total Post Cliks", 'Total Post Attachment Click', 'Total Post Likes Clikes');
                    $coreValueCountCol = "D";
                    $countCol = 3;
                } else if ($type == 'endorsed') {
                    $result = array("nDorser", "nDorsement Date", "CORE VALUES EMBODIED");
