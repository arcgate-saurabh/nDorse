...
//            }
    ...
    public function forgotPasswordEmails() {
        ...
        $params['fields'] = array("*");
        $params['limit'] = 5;
        $params['conditions'] = array('email_sent' => 0, 'expired' => 0);
        $params['joins'] = array(
            array(
                'table' => 'users',
                'alias' => 'User',
...
//        echo $this->PasswordCode->getLastQuery();	
//        pr($params); 
//        pr($verifications); exit;
...
//                    pr($verification['PasswordCode']);

                    $endcodedID = $verification['PasswordCode']['user_id_encpt'];
                    $endcodedCODE = $verification['PasswordCode']['code'];
                    $link = $endcodedID . "@$@" . $endcodedCODE;

                    $resetPasswordURL = Router::url('/', true) . "client/resetpasswordset/" . $link;

                    

//                    echo $resetPasswordURL; exit;
                    if (strpos($resetPasswordURL, 'localhost') < 0) {
                        $resetPasswordURL = str_replace("http", "https", $resetPasswordURL);
                    }
                    
                    if (strpos($resetPasswordURL, 'api') > 0) {
                        $resetPasswordURL = "https://www.ndorse.net/client/resetpasswordset/" . $link;
                    }
                    
                    $resetPasswordIOSURL = str_replace("https", "applinks", $resetPasswordURL);

                    $viewVars["verification_link"] = $resetPasswordURL;
                    $viewVars["verification_link_ios"] = $resetPasswordIOSURL;
