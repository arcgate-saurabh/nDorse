...
//            $location = dirname(__FILE__) . '/' . 'Certificates_new_1.pem';
            //$location = dirname(__FILE__) . '/' . 'Certificates.pem';
            $location = dirname(__FILE__) . '/' . 'pushcert_new_1.pem';
...
//            $EmailModel->saveMany($emailQueue);
        ...
        if (!empty($userid)) {
            ...
            foreach ($loginstatisticsdata as $datalogin) {
                ...
                if ($token != "" && strtolower($datalogin["LoginStatistics"]["os"]) == "ios") {
                    $deviceToken_msg_arr_ios[] = array('token' => $token, 'count' => '1', 'msg' => $content, "data" => $parameter, 'original_msg' => $content_1);
                    ...
                    //       "title" => "nDorse App");
                    $deviceToken_msg_arr_android[] = array('token' => $token, 'count' => '1', 'msg' => $content_1, "data" => $parameter);
                }
            }
    ...
    function mailstoorganizationsusers($emailsarray, $content, $attachment) {
...
//                    echo $res->success;
    ...
    function announcementspostdata($organizationslist, $content, $attachment = "") {
        ...
        if (!empty($organizationslist)) {
            ...
            if (!empty($emailsarray)) {
                $this->mailstoorganizationsusers($emailsarray, $content, $attachment);
