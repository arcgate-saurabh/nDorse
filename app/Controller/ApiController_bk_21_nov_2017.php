...
// @TODO : email and push notification to endorsee
        ...
        if ($this->request->is('post')) {
            ...
            $post['title'] = isset($this->request->data['title']) ? $this->request->data['title'] : "";
...
//}
        if ($model == "OrgJobTitles") {
            ...
            $fields = array("name", "allow_comments", "optional_comments", "allow_attachment","public_endorse_visible");
        } else {
            $fields[] = "name";
            $title = "name";
        }
...
//                        ));
                        ...
                        $viewVars = serialize(array("org_name" => $organization['name'], 'user' => $loggedinUser));
                        $to = $admin['User']['email'];
                    ...
                    if (isset($value["PostAttachments"]) && !empty($value["PostAttachments"])) {
                        foreach ($value["PostAttachments"] as $index => $postAttchment) {
                            if ($postAttchment['type'] == 'emojis') {
                                $postAttachmentemoji[] = Router::url('/', true) . EMOJIS_IMAGE_DIR . $postAttchment['name'];
                            } else if ($postAttchment['type'] == 'image') {
                                $postAttachmentimg[] = Router::url('/', true) . "app/webroot/" . POST_IMAGE_DIR . $postAttchment['name'];
                            } else if ($postAttchment['type'] == 'files') {
                                $postAttachmentFiles[] = Router::url('/', true) . "app/webroot/" . POST_FILE_DIR . $postAttchment['name'];
                            }
                        }
...
//                    }
        ...
        if (!empty($res[0])) {
            ...
            if (!empty($userinfo)) {
                if (isset($userinfo[0]['User']['image'])) {
                    $userImage = Router::url('/', true) . "app/webroot/" . PROFILE_IMAGE_DIR . "small/" . $userinfo[0]['User']['image'];
                }
    ...
    public function wallPostdetails() {
        ...
        if ($this->request->is('post')) {
            ...
            $user_id = $loggedinUser["id"];
            $params = array();
            $params['fields'] = "*,UNIX_TIMESTAMP() as curr_time,UNIX_TIMESTAMP(created) as created";
...
//            $passCode = $this->PasswordCode->find("first", array("conditions" => array("code" => $this->request->data['verification_code'], "email" => $this->request->data['email'])));
    ...
    function sendPushNotification($deviceToken_msg_arr = null) {
        ...
        $deviceToken_msg_arr[0]['token'] = '0dbe68105122d9977243b9b570777fa60e508dfc67e98948e3fd827e9bcc16cc';
