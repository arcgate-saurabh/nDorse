...
$(window).scroll(function () {
    ...
    if ($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
        ...
        if (endorsepage <= totalendorsepage) {
            curl = siteurl + 'cajax/getendorsedatesearch'; //Change to get pending post data on scroll
...
$(document).on("click", ".live-feeds-post", function (event) {
    ...
    var clnew = $(event.target).attr('class');
    if (clnew == undefined) {
        window.location.href = siteurl + "post/edit/" + $(this).attr("post_id");
...
/** Added by Babulal Prasad @23122017 
